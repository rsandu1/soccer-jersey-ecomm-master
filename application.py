...
def filter():
    if request.args.get('continent'):
        ...
        shirts= db.execute("SELECT * FROM shirts WHERE instr(UPPER(team), UPPER(:query)) ORDER BY team ASC", query=query)
    shirtsLen = len(shirts)
    # Initialize shopping cart variables
    shoppingCart = []
    shopLen = len(shoppingCart)
...
@app.route("/completepurchase", methods=["GET"])
...
def logged():
    ...
    if user == "" or pwd == "" or user.isalnum() == False or pwd.isalnum() == False:
        return render_template ( "login.html" )
...
@app.route("/verifypurchase/", methods=["POST"] )
def verifypurchase():
    # Get log in info from log in form
    user = request.form["username"].lower()
    pwd = request.form["password"]
    ...
    return render_template ( "checkout.html", msg="Can't Complete Purchase. Wrong Credentials. Try Again." )
